...
TASK=pick_v39_shoulder_precision
...
"$PYTHON" scripts/benchmark_visual_servo_pick.py \
  ...
  --seed 777 2>&1 | tee "$LOG_DIR/pick_v39_visual_expert_bench.log"
...
if [[ ! -f "$DATA_ROOT/.record_complete" ]]; then
  "$PYTHON" scripts/record_pick_episodes.py \
    ...
    --seed 3701 2>&1 | tee "$LOG_DIR/pick_v39_record.log"
...
"$PYTHON" scripts/validate_dataset_replay.py \
  ...
  --num-episodes 200 \
  --seed 3702 \
  --min-success-rate 0.95 2>&1 | tee "$LOG_DIR/pick_v39_replay.log"
...
if [[ ! -f "$CACHE_DIR/.cache_complete" ]]; then
  "$PYTHON" scripts/build_frame_cache.py \
    ...
    --seed 3803 2>&1 | tee "$LOG_DIR/pick_v39_cache.log"
...
stage 5/8 "v39 shoulder-precision continuation with native closed-loop selection"
if [[ ! -f "$CKPT_DIR/${TASK}_final.pt" ]]; then
  "$PYTHON" scripts/train.py \
    --data-root "$DATA_ROOT" \
    --frame-cache "$CACHE_DIR" \
    --task "$TASK" \
    --init-from "$ROOT/checkpoints/pick_v38_broad_cache/pick_v38_broad_cache_best.pt" \
    --init-from-raw \
    --use-dinov2 \
    --img-size 224 \
    --chunk-size 16 \
    ...
    --late-bc-weight 4.0 \
    --late-bc-frac 0.35 \
    --ema-decay 0 2>&1 | tee "$LOG_DIR/pick_v39_train.log"
else
...
printf '%s\n' "$SELECTED_CKPT" | tee "$LOG_DIR/pick_v39_selected_checkpoint.txt"

stage 6/8 "Shoulder-pan visual sensitivity gate"
...
"$PYTHON" scripts/benchmark_random_pick.py \
  ...
  --max-steps 450 \
  --seed 3905 \
  --raw-weights 2>&1 | tee "$LOG_DIR/pick_v39_random_bench.log"
...
"$PYTHON" scripts/eval_pick_ball.py \
  ...
  --out "$VIDEO_DIR/eval_pick_v39_shoulder_precision.mp4" 2>&1 | tee "$LOG_DIR/pick_v39_video.log"

echo
echo "=== V39 SHOULDER-PRECISION PIPELINE COMPLETE ==="
echo "selected checkpoint: $SELECTED_CKPT"
echo "benchmark log: $LOG_DIR/pick_v39_random_bench.log"
echo "video: $VIDEO_DIR/eval_pick_v39_shoulder_precision.mp4"
df -h "$ROOT"
